...
//          faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], "blur_blend_ratio", 1);
    ...
    public int onDrawFrameFBODoubleInput(byte[] img, int texId, int texWidth,
                                         int texHeight) {
        if (!isActive) {
            return texId;
        }
        ...
        if (mCurrentCameraType == Camera.CameraInfo.CAMERA_FACING_FRONT) {
            ...
            int fuTex = onDrawFrameDoubleInput(img, fboTex[0], texHeight, texWidth);
            ...
            int fuTex = onDrawFrameDoubleInput(img, fboTex[1], texHeight, texWidth);
