...
import com.faceunity.nama.gles.FullFrameRect;
import com.faceunity.nama.gles.Texture2dProgram;
...
public class FURenderer implements IFURenderer, IModuleManager {
    ...
    private int createRotationMode() {
        ...
        Log.e(TAG, "createRotationMode: deviceOrientation " + deviceOrientation + " cameraType " + cameraType);
    ...
    private float[] mRotated270Matrix = new float[16];
    private float[] mRotated90Matrix = new float[16];
    private float[] mTransformMatrix = new float[16];
    private int[] mOriginalViewport = new int[4];

    public void loadItems() {
        Matrix.setIdentityM(mRotated270Matrix, 0);
        Matrix.rotateM(mRotated270Matrix, 0, 270, 0, 0, 1);
        Matrix.setIdentityM(mRotated90Matrix, 0);
        Matrix.rotateM(mRotated90Matrix, 0, 90, 0, 0, 1);
    ...
    public int onDrawFrameDualInputFBO(byte[] img, int texId, int texWidth, int texHeight) {
        createFBO(texWidth, texHeight);

        GLES20.glGetIntegerv(GLES20.GL_VIEWPORT, mOriginalViewport, 0);
        ...
        mFullFrameRect.drawFrame(texId, isFront ? mRotated270Matrix : mTransformMatrix);
        GLES20.glBindFramebuffer(GLES20.GL_FRAMEBUFFER, 0);
        GLES20.glViewport(mOriginalViewport[0], mOriginalViewport[1], mOriginalViewport[2], mOriginalViewport[3]);
        int fuTex;
        if (img == null) {
            fuTex = onDrawFrameSingleInput(mFboTex[0], texHeight, texWidth);
        } else {
            fuTex = onDrawFrameDualInput(img, mFboTex[0], texHeight, texWidth);
        }

        GLES20.glBindFramebuffer(GLES20.GL_FRAMEBUFFER, mFboId[1]);
        GLES20.glViewport(0, 0, texWidth, texHeight);
        mFullFrameRect.drawFrame(fuTex, isFront ? mRotated90Matrix : mTransformMatrix);
        GLES20.glBindFramebuffer(GLES20.GL_FRAMEBUFFER, 0);
        GLES20.glViewport(mOriginalViewport[0], mOriginalViewport[1], mOriginalViewport[2], mOriginalViewport[3]);
    ...
    private static final int FBO_COUNT = 2;
    private int[] mFboId;
    private int[] mFboTex;
    private int[] mRboId;
    ...
    private void createFBO(int width, int height) {
        if (mFboWidth != width || mFboHeight != height) {
            deleteFBO();
        }
        if (mFboTex != null) {
            return;
        ...
        mFboWidth = width;
        mFboHeight = height;
        Log.d(TAG, "createFBO() width = [" + width + "], height = [" + height + "]");

        mFboId = new int[FBO_COUNT];
        mFboTex = new int[FBO_COUNT];
        mRboId = new int[FBO_COUNT];

//generate fbo id
        GLES20.glGenFramebuffers(FBO_COUNT, mFboId, 0);
//generate texture
        GLES20.glGenTextures(FBO_COUNT, mFboTex, 0);
//generate render buffer
        GLES20.glGenRenderbuffers(FBO_COUNT, mRboId, 0);

        for (int i = 0; i < mFboId.length; i++) {
//Bind Frame buffer
            GLES20.glBindFramebuffer(GLES20.GL_FRAMEBUFFER, mFboId[i]);
//Bind texture
            GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, mFboTex[i]);
...
//Attach texture FBO color attachment
            GLES20.glFramebufferTexture2D(GLES20.GL_FRAMEBUFFER, GLES20.GL_COLOR_ATTACHMENT0, GLES20.GL_TEXTURE_2D, mFboTex[i], 0);
//Attach render buffer to depth attachment
            GLES20.glFramebufferRenderbuffer(GLES20.GL_FRAMEBUFFER, GLES20.GL_DEPTH_ATTACHMENT, GLES20.GL_RENDERBUFFER, mRboId[i]);
//we are done, reset
    ...
    private void deleteFBO() {
        if (mFboId == null || mFboTex == null || mRboId == null) {
            return;
        }
        Log.d(TAG, "deleteFBO: ");
        GLES20.glDeleteFramebuffers(FBO_COUNT, mFboId, 0);
        GLES20.glDeleteTextures(FBO_COUNT, mFboTex, 0);
        GLES20.glDeleteRenderbuffers(FBO_COUNT, mRboId, 0);
        mFboId = null;
        mFboTex = null;
        mRboId = null;
    }
